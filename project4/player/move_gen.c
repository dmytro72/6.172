...
int generate_all(position_t* p, sortable_move_t* sortable_move_list,
  ...
  // 1 = path of enemy laser with no moves
  // 2 = path of friendly laser with no moves
  mark_laser_path(p, opp_color(color_to_move), laser_map, 1);
  mark_laser_path(p, color_to_move, laser_map, 2);
  ...
  for (fil_t f = 0; f < BOARD_WIDTH; f++) {
    for (rnk_t r = 0; r < BOARD_WIDTH; r++) {
      ...
      case PAWN:
        if (laser_map[sq] & 1) {
          continue;  // Piece is zombified by enemy laser.
        }
      case KING:
        if (color != color_to_move && (laser_map[sq] & 2) != 2) {  // Wrong color and not zombified.
          break;
        }
        // directions
        for (int d = 0; d < 8; d++) {
